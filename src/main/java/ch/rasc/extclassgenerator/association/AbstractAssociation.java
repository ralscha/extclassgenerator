...
public abstract class AbstractAssociation {
	...
	public static AbstractAssociation createAssociation(ModelAssociation associationAnnotation) {
		...
		if (type == ModelAssociationType.HAS_MANY) {
			...
			if (StringUtils.hasText(associationAnnotation.name())) {
				LogFactory.getLog(ModelGenerator.class).warn(getWarningText(null, name, association.getType(), "name"));
			}
		} else {
	...
	public static AbstractAssociation createAssociation(ModelAssociation associationAnnotation, ModelBean model,
		...
		if (type == ModelAssociationType.HAS_MANY) {
			...
			if (StringUtils.hasText(associationAnnotation.setterName())) {
				LogFactory.getLog(ModelGenerator.class).warn(
						getWarningText(declaringClass, name, association.getType(), "setterName"));
			}

			if (StringUtils.hasText(associationAnnotation.getterName())) {
				LogFactory.getLog(ModelGenerator.class).warn(
						getWarningText(declaringClass, name, association.getType(), "getterName"));
			}
			...
			if (associationAnnotation.autoLoad()) {
				LogFactory.getLog(ModelGenerator.class).warn(
						getWarningText(declaringClass, name, association.getType(), "autoLoad"));
			}
			if (StringUtils.hasText(associationAnnotation.name())) {
				LogFactory.getLog(ModelGenerator.class).warn(
						getWarningText(declaringClass, name, association.getType(), "name"));
			}
		} else {
			HasOneAssociation hasOneAssociation = (HasOneAssociation) association;
